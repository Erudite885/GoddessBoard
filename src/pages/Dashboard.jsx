...
const Dashboard = () => {
  ...
  const { data, isLoading, isFetching, error } = useGetSearchQuery({
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {isFetching ? (
              ...
              jobs
                .slice(0, 4)
                .map((job) => (
                  <JobCard key={job?.job_id} job={job} jobs={jobs} />
                ))
            )}
          </div>
        </div>
        ...
        <div className="basis-full px-1 lg:basis-[32%]">
          ...
          <div className="bg-white dark:bg-black_2  border-white rounded-md overflow-y-scroll scroll-smooth h-96 sm:h-[500px] lg:h-[800px]">
            {jobs.slice(4).map((job) => (
              <JobSmallCard key={job?.job_id} job={job} />
            ))}
          </div>
        </div>
          ...
          <div className="bg-white dark:bg-black_2 rounded-md p-1">
            <div className="m-4">
              <SectionHeader
                ...
                onOptionSelect={setSortMeetingsBy}
              />
            </div>
            {meets.map((meet) => (
