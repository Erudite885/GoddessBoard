...
export const currentSearch = createSlice({
  ...
  reducers: {
    ...
    selectTypeRemove: (state, action) => {
      state.employmentTypesArray = state.employmentTypesArray.filter(
        (item) => item !== action.payload
      );
    },
    ...
    selectJobRequirementRemove: (state, action) => {
      state.jobRequirementArray = state.jobRequirementArray.filter(
        (item) => item !== action.payload
      );
    },
    ...
    selectSalaryRemove: (state, action) => {
      state.salaryBounds = state.salaryBounds.filter(
        (item) => item.min !== action.payload.min
      );
    },
